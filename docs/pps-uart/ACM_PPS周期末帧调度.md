...
2026-09-22：RK3506网关已增加GPIO1_C2（gpiochip1/18）实际脉冲校验，详见
[GPIO脉冲与超帧校验](GPIO脉冲与超帧校验.md)。下述软件计数仍用于判断候选末帧，
实际GPIO期限用于放行ACM；丢失1～4周期保留请求，连续丢失5周期退出。
...
未启用GPIO监测的调用者仍只依赖首次外部启动与连续S0序列。RK3506网关启用监测后，
使用内核GPIO单调时间戳交叉校验帧计数和超帧位置，并独立监测脉冲丢失；首次S0
改由GPIO相位校验，ACM等待恢复允许跨过丢失周期。若脉冲正常但完全没有S0，仍沿用
网关原有IRQ停滞检查。真实延迟与终端业务连续性仍需板端验证。
