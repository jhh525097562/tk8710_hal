...
static TK8710_MAYBE_UNUSED int tk8710_noise_process(const char* data_dir, int rate_mode, const char* output_file) {
    ...
    printf("[噪底计算] FFT 配置:\n");
    printf("[噪底计算]   速率模式: %d\n", config.rate_mode);
    printf("[噪底计算]   FFT 长度: %d\n", config.fft_len);
    printf("[噪底计算]   原始数据长度: %d\n", config.data_len);
    printf("[噪底计算]   天线数量: %d\n", config.num_antennas);
    ...
    if (tk8710_fft_process_with_noise(antenna_data, noise_floor,
                                       ...
                                       &config.para, 1) != 0) {
        printf("[噪底计算] 错误: FFT 处理失败\n");
        free(antenna_data);
        return -1;
    }
    printf("[噪底计算] FFT 处理完成\n");
    ...
    if (tk8710_save_noise_floor(noise_floor, config.num_antennas, output_file) != 0) {
        printf("[噪底计算] 错误: 保存结果失败\n");
        return -1;
    }
    
    /* 打印结果 (dBm 格式) */
    printf("[噪底计算] === 噪底结果 (dBm) ===\n");
    for (int ant = 0; ant < config.num_antennas; ant++) {
        float noise_dbm = 10.0f * log10f(noise_floor[ant]) + TK8710_ANOISE_OFFSET;
        printf("[噪底计算]   天线 %d: %.4f dBm\n", ant + 1, noise_dbm);
    }
    printf("[噪底计算] ========================\n");
