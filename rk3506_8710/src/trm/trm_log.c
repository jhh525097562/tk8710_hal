...
static void trm_sanitize_ascii(char* text)
{
...
#if defined(PLATFORM_JTOOL) || defined(PLATFORM_TMS570)
...
#else
    return snprintf(buffer, size, "%lu", (unsigned long)TK8710GetTickMs());
#endif
}
...
void TRM_LogInit(TRMLogLevel level)
{
    ...
    TRM_LogOutput(TRM_LOG_INFO, "TRM", __FILE__, __LINE__, __func__,
                  "TRM log system initialized - level:%s", TRM_LogGetLevelName(level));
}
...
int TRM_LogConfig(TRMLogLevel level, uint8_t enable_file_logging)
{
    ...
    /* 配置并启用文件日志 */
    if (enable_file_logging && TK8710_PLATFORM_HAS_FILE_IO) {
        TRM_LogEnableFileLogging(1, NULL);
    }
    
    TRM_LogOutput(TRM_LOG_INFO, "TRM", __FILE__, __LINE__, __func__,
                  "TRM log system configured - level:%s, fileLogging:%s",
