...
static uint8_t g_manualTxPowerDiagCount = 0;
static uint8_t g_zeroS1TxPrepareDiagCount = 0;
...
static uint8_t g_slaveBcnTrackingActive = 0U;
static uint8_t g_slaveBcnReceivedInFrame = 0U;
static uint8_t g_slaveBcnMissFrameCount = 0U;
static uint8_t g_slaveContiScanState = 0xFFU;
                               ...
                               uint16_t expectedLen)
{
    ...
    memcpy(newData, txBuffer->data, oldLen);
    for (i = oldLen; i < expectedLen; i++) {
...
static uint32_t tk8710_read_be32(const uint8_t* data)
{
...
static uint64_t tk8710_read_be40(const uint8_t* data)
{
...
static void tk8710_write_be32(uint8_t* data, uint32_t value)
{
...
static void tk8710_write_be40(uint8_t* data, uint64_t value)
{
    value &= 0xFFFFFFFFFFULL;
    data[0] = (uint8_t)(value >> 32);
...
static void tk8710_process_tx_prepare(void);
static void tk8710_process_zero_s1_tx_after_callback(uint8_t irqType);
...
static int tk8710_set_slave_conti_scan(uint8_t enable)
{
    ...
    current = (uint8_t)TK8710_S_INIT_5_CONTI_SCAN_GET(init5.data);
    if (current != enable) {
        init5.data = TK8710_S_INIT_5_CONTI_SCAN_SET(init5.data, enable);
...
static void tk8710_reset_slave_bcn_tracking(void)
{
    g_slaveBcnTrackingActive = 0U;
    g_slaveBcnReceivedInFrame = 0U;
    g_slaveBcnMissFrameCount = 0U;
...
static uint8_t tk8710_should_call_tx_slot_callback(uint8_t irqType)
{
    ...
    if (slotCfg == NULL) {
        return (irqType == TK8710_IRQ_S1) ? 1U : 0U;
    }

    if (slotCfg->s1Cfg[0].byteLen == 0U) {
        if (slotCfg->msMode == TK8710_MODE_MASTER) {
            return (irqType == TK8710_IRQ_S0) ? 1U : 0U;
        }
        if (slotCfg->msMode == TK8710_MODE_SLAVE) {
            return (irqType == TK8710_IRQ_S2) ? 1U : 0U;
        }
    }
...
static void tk8710_process_tx_prepare(void)
{
    ...
    g_irqResult.autoTxValid = 0U;
    tk8710_s1_auto_tx_process();

    if (slotCfg->txBeamCtrlMode == 1U) {
        if (g_simulationDataLoaded != 0U) {
            uint32_t userValRegs[4] = {0xFFFFFFFFU, 0xFFFFFFFFU,
            ...
            for (reg = 0; reg < 4; reg++) {
                ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL,
                                     MAC_BASE + 0x3CU + (uint32_t)reg * 4U,
                                     userValRegs[reg]);
                if (ret != TK8710_OK) {
                    TK8710_LOG_IRQ_ERROR("Failed to set MAC user_val%d: %d", reg, ret);
                }
            }
            brdUserVal.data = TK8710_S_INIT_17_ENCODE(0xFFFFU);
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL,
                                 MAC_BASE + offsetof(struct mac, init_17),
                                 brdUserVal.data);
            if (ret != TK8710_OK) {
                TK8710_LOG_IRQ_ERROR("Failed to set init_17: %d", ret);
            }
        } else {
...
static void tk8710_process_zero_s1_tx_after_callback(uint8_t irqType)
{
    ...
    if ((slotCfg == NULL) || (slotCfg->s1Cfg[0].byteLen != 0U)) {
        return;
    }

    if ((slotCfg->msMode == TK8710_MODE_MASTER) &&
        (irqType == TK8710_IRQ_S0)) {
        prepare = 1U;
    } else if ((slotCfg->msMode == TK8710_MODE_SLAVE) &&
               (irqType == TK8710_IRQ_S2)) {
        prepare = 1U;
    }

    if (prepare != 0U) {
        if (g_zeroS1TxPrepareDiagCount < 8U) {
            TK8710_LOG_IRQ_WARN("Zero-S1 TX prepare after IRQ %u callback", irqType);
            g_zeroS1TxPrepareDiagCount++;
        }
        tk8710_process_tx_prepare();
    }
}
        ...
        if (irqStatus & (1 << i) && g_irqHandlers[i]) {
            ...
            if (g_useMultiCallbacks) {
                ...
                switch (i) {
                    ...
                    case TK8710_IRQ_S1:
                    case TK8710_IRQ_S2:
                        if (tk8710_should_call_tx_slot_callback((uint8_t)i) &&
                            g_driverCallbacks.onTxSlot) {
                            g_driverCallbacks.onTxSlot(&g_irqResult);
                            tk8710_process_zero_s1_tx_after_callback((uint8_t)i);
...
void TK8710ClearIrqStatus(uint32_t mask)
{
    ...
    /* 根据mask设置对应的清除位 */
    if (mask & (1 << TK8710_IRQ_RX_BCN))   irqCtrl1.data = TK8710_S_IRQ_CTRL1_RXBCN_INTR_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_BRD_UD))   irqCtrl1.data = TK8710_S_IRQ_CTRL1_BRD_UD_INTR_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_BRD_DATA)) irqCtrl1.data = TK8710_S_IRQ_CTRL1_BRD_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_MD_UD))    irqCtrl1.data = TK8710_S_IRQ_CTRL1_DATA_UD_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_MD_DATA))  irqCtrl1.data = TK8710_S_IRQ_CTRL1_DATA_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_S0))       irqCtrl1.data = TK8710_S_IRQ_CTRL1_S0_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_S1))       irqCtrl1.data = TK8710_S_IRQ_CTRL1_S1_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_S2))       irqCtrl1.data = TK8710_S_IRQ_CTRL1_S2_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_S3))       irqCtrl1.data = TK8710_S_IRQ_CTRL1_S3_IRQ_CLR_SET(irqCtrl1.data, 1U);
    if (mask & (1 << TK8710_IRQ_ACM))      irqCtrl1.data = TK8710_S_IRQ_CTRL1_ACM_IRQ_CLR_SET(irqCtrl1.data, 1U);
...
void TK8710EnableIrq(uint32_t mask)
{
    ...
    if (mask & (1 << TK8710_IRQ_RX_BCN))   irqCtrl0.data = TK8710_S_IRQ_CTRL0_RXBCN_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_BRD_UD))   irqCtrl0.data = TK8710_S_IRQ_CTRL0_BRD_UD_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_BRD_DATA)) irqCtrl0.data = TK8710_S_IRQ_CTRL0_BRD_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_MD_UD))    irqCtrl0.data = TK8710_S_IRQ_CTRL0_MD_UD_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_MD_DATA))  irqCtrl0.data = TK8710_S_IRQ_CTRL0_MD_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_S0))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S0_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_S1))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S1_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_S2))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S2_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_S3))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S3_IRQ_MASK_SET(irqCtrl0.data, 0U);
    if (mask & (1 << TK8710_IRQ_ACM))      irqCtrl0.data = TK8710_S_IRQ_CTRL0_ACM_IRQ_MASK_SET(irqCtrl0.data, 0U);
...
void TK8710DisableIrq(uint32_t mask)
{
    ...
    if (mask & (1 << TK8710_IRQ_RX_BCN))   irqCtrl0.data = TK8710_S_IRQ_CTRL0_RXBCN_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_BRD_UD))   irqCtrl0.data = TK8710_S_IRQ_CTRL0_BRD_UD_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_BRD_DATA)) irqCtrl0.data = TK8710_S_IRQ_CTRL0_BRD_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_MD_UD))    irqCtrl0.data = TK8710_S_IRQ_CTRL0_MD_UD_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_MD_DATA))  irqCtrl0.data = TK8710_S_IRQ_CTRL0_MD_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_S0))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S0_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_S1))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S1_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_S2))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S2_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_S3))       irqCtrl0.data = TK8710_S_IRQ_CTRL0_S3_IRQ_MASK_SET(irqCtrl0.data, 1U);
    if (mask & (1 << TK8710_IRQ_ACM))      irqCtrl0.data = TK8710_S_IRQ_CTRL0_ACM_IRQ_MASK_SET(irqCtrl0.data, 1U);
...
void TK8710PrintIrqTimeStats(void)
{
    ...
    TK8710_LOG_IRQ_INFO("=== IRQ processing time statistics ===");
    TK8710_LOG_IRQ_INFO("%-8s %-8s %-8s %-8s %-8s %-12s", 
                        "Type", "Count", "Total", "Average", "Max", "Min");
    TK8710_LOG_IRQ_INFO("------------------------------------------------");
...
static int tk8710_configure_multi_rate(uint8_t rateIndex)
{
    ...
    init9.data = TK8710_S_INIT_9_ENCODE(slotCfg->antEn,
                                        slotCfg->rfSel,
                                        (rateIndex % slotCfg->rateCount == 0U) ? slotCfg->txBcnAntEn : 0U);
    ...
    macConfig0Data = TK8710_S_MAC_CONFIG_0_ENCODE(0U,
                                                  slotCfg->s1Cfg[rateIndex].byteLen,
                                                  slotCfg->rateModes[rateIndex]);
    ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
    ...
    macConfig1Data = TK8710_S_MAC_CONFIG_1_ENCODE(slotCfg->s2Cfg[rateIndex].byteLen,
                                                  slotCfg->s3Cfg[rateIndex].byteLen,
                                                  slotCfg->plCrcEn);
    ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
    ...
    /* 配置 init_2: da1_m */
    init2.data = TK8710_S_INIT_2_ENCODE(slotCfg->s1Cfg[rateIndex].da_m);  /* da1_m from S1[rateIndex] */
    ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
    ...
    init3.data = TK8710_S_INIT_3_ENCODE(slotCfg->s2Cfg[rateIndex].da_m,
                                        slotCfg->txBeamCtrlMode,
                                        0U);  /* da2_m from S2[rateIndex], tx_fix_freq */
    ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
    ...
    /* 配置 init_4: da3_m */
    init4.data = TK8710_S_INIT_4_ENCODE(slotCfg->s3Cfg[rateIndex].da_m);  /* da3_m from S3[rateIndex] */
    ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
    ...
    {
        s_init_18 init18;
        init18.data = TK8710_S_INIT_18_ENCODE(slotCfg->s0Cfg[rateIndex].da_m);  /* da0_m from S0[rateIndex] */
        ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
...
static void tk8710_handle_rx_bcn(void)
{
    ...
    if (TK8710GetWorkType() == TK8710_MODE_SLAVE) {
        ...
        g_slaveBcnTrackingActive = 1U;
        g_slaveBcnReceivedInFrame = 1U;
        g_slaveBcnMissFrameCount = 0U;
        if (tk8710_set_slave_conti_scan(0U) != TK8710_OK) {
            g_slaveContiScanState = 0xFFU;
        }
    }
    ...
    /* 读取BCN频偏和BCN bits */
    {
        ...
        /* 设置中断结果中的BCN信息 */
        g_irqResult.bcn_freq_offset = (int16_t)TK8710RegPackGetSigned(bcnObv2.data,
                                                                      TK8710_S_BCN_OBV2_FREQ_OFFSET_SHIFT,
                                                                      TK8710_S_BCN_OBV2_FREQ_OFFSET_WIDTH);
        g_irqResult.rx_bcnbits = TK8710_S_BCN_OBV2_BCN_BITS_OUT_GET(bcnObv2.data);
        g_irqResult.rxbcn_status = TK8710_S_BCN_OBV1_SYNC_ON_GET(bcnObv1.data);  /* 同步状态 */
        
        /* 打印读取到的BCN信息 */
        TK8710_LOG_IRQ_INFO("BCN Info: bits=%u, freq_offset=%d, q=%u, sync=%u", 
                           TK8710_S_BCN_OBV2_BCN_BITS_OUT_GET(bcnObv2.data),
                           g_irqResult.bcn_freq_offset,
                           TK8710_S_BCN_OBV1_BCN_Q_GET(bcnObv1.data),
                           TK8710_S_BCN_OBV1_SYNC_ON_GET(bcnObv1.data));
    }
...
{
    int ret;
    uint8_t* rxBuf = g_irqInfoScratch;
    uint8_t maxUsers;
    uint8_t dataUserNum;
    uint8_t i;
    ...
    if (ret == 0) {
        for (i = 0; i < maxUsers; i++) {
            g_userInfoRxBuffers[i].freq = tk8710_read_be32(&rxBuf[i * 4U]);
    ...
    if (ret == 0) {
        for (i = 0; i < maxUsers; i++) {
            ...
            for (int ant = 0; ant < 8; ant++) {
                /* 直接按字节顺序获取AH数据，不做调整 */
                uint64_t ah40 = tk8710_read_be40(&rxBuf[(i * 40U) + ((uint32_t)ant * 5U)]);
    ...
    if (ret == 0) {
        for (i = 0; i < maxUsers; i++) {
            uint64_t pilotPower = tk8710_read_be40(&rxBuf[i * 5U]) & 0x0FFFFFFFFFFULL;  /* 40位功率, 取低40位 */
    ...
    /* 获取8根天线的ANoise信息 */
    uint8_t* aNoiseData = g_irqANoiseScratch;  /* 8根天线，每根4字节（32位） */
    ret = TK8710SpiGetInfo(TK8710_GET_INFO_ANOISE, aNoiseData, 32U);
    if (ret == TK8710_OK) {
...
//    /* 标记数据有效 */
//    if (maxUsers >= 2U) {
//        TK8710_LOG_IRQ_INFO("MD UD user0 freq=%08X ah0_i=%08X ah0_q=%08X pilot=%08X",
//                            g_userInfoRxBuffers[0].freq,
//                            g_userInfoRxBuffers[0].ahData[0],
//                            g_userInfoRxBuffers[0].ahData[1],
//                            (uint32_t)(g_userInfoRxBuffers[0].pilotPower & 0xFFFFFFFFULL));
//        TK8710_LOG_IRQ_INFO("MD UD user1 freq=%08X ah0_i=%08X ah0_q=%08X pilot=%08X",
//                            g_userInfoRxBuffers[1].freq,
//                            g_userInfoRxBuffers[1].ahData[0],
//                            g_userInfoRxBuffers[1].ahData[1],
//                            (uint32_t)(g_userInfoRxBuffers[1].pilotPower & 0xFFFFFFFFULL));
//    }
...
static void tk8710_md_data_process(void)
{
    ...
    /* 解析CRC结果 */
    TK8710_LOG_IRQ_INFO("MD DATA CRC regs: %08X %08X %08X %08X",
                        crcRegs[0], crcRegs[1], crcRegs[2], crcRegs[3]);
...
static void tk8710_s0_bcn_rotation_process(void)
{
    ...
    for (int ant = 0; ant < 8; ant++) {
        ...
        config28.data = TK8710_S_TX_CONFIG_28_ENCODE(scDa, scBcn, 0U, 0U, 0U);
...
static void tk8710_handle_slot1(void)
{
    ...
    /* Zero-length S1 is prepared after the selected S0/S2 TX callback. */
    if ((slotCfg != NULL) && (slotCfg->s1Cfg[0].byteLen == 0U)) {
        return;
    }
    ...
    for (i = 0; i < maxUsers; i++) {
        if (g_txBuffers[i].valid) {
            ...
            if (dataLen < expectedLen) {
                ...
                TK8710_LOG_IRQ_DEBUG("User[%u] TX data truncated: actual=%u expected=%u",
            ...
            if (ret == TK8710_OK) {
                ...
                tx_pow_ctrl.data = TK8710_S_TX_POW_CTRL_ENCODE(txPower, userIndex);
...
static void tk8710_handle_slot2(void)
{
    ...
    if (TK8710GetWorkType() == TK8710_MODE_SLAVE &&
        g_slaveBcnTrackingActive != 0U) {
        if (g_slaveBcnReceivedInFrame != 0U) {
            g_slaveBcnReceivedInFrame = 0U;
            g_slaveBcnMissFrameCount = 0U;
            ...
            if (g_slaveBcnMissFrameCount == SLAVE_BCN_MISS_FRAME_THRESHOLD) {
                ...
                if (tk8710_set_slave_conti_scan(1U) != TK8710_OK) {
                    g_slaveBcnMissFrameCount--;
                    g_slaveContiScanState = 0xFFU;
                } else {
...
static void tk8710_handle_slot3(void)
{
    ...
    // if(g_irqCounters[TK8710_IRQ_S3] % 2 == 0){
    //     TK8710GpioSet("gpiochip0", 4, 0);
    // }else{
    //     TK8710GpioSet("gpiochip0", 4, 1);
    // }
...
static void tk8710_s1_manual_tx_process(void)
{
    ...
    if (slotCfg->brdUserNum > 0) {
        ...
        /* 配置广播用户valid位 (MAC init_17寄存器) */
        {
            ...
            brdUserVal.data = TK8710_S_INIT_17_ENCODE((1U << slotCfg->brdUserNum) - 1U);
    ...
    /* 1. 配置频率 - 广播和用户数据合并传输 */
    {
        ...
        for (uint8_t i = 0; i < validUserCount; i++) {
            ...
            if (g_forceMaxUsersTx && !g_txBuffers[userIndex].valid) {
                ...
                tk8710_write_be32(&spiBuffer[writeLen * 4U], testFreq);
                ...
                tk8710_write_be32(&spiBuffer[writeLen * 4U], freq);
        ...
        for (uint8_t i = 0; i < validUserCount; i++) {
            ...
            if (g_forceMaxUsersTx && !g_txBuffers[userIndex].valid) {
                ...
                for (int ant = 0; ant < 8; ant++) {
                    ...
                    tk8710_write_be40(&spiBuffer[writeLen * 40U + (uint16_t)ant * 5U], ah40);
                }
    ...
    /* 3. 配置Pilot功率 - 广播和用户数据合并传输 */
    {
        ...
        for (uint8_t i = 0; i < validUserCount; i++) {
            ...
            uint64_t pilotPower = g_userInfoTxBuffers[userIndex].pilotPower;

            tk8710_write_be40(&spiBuffer[writeLen * 5U], pilotPower);

            TK8710_LOG_IRQ_DEBUG("Manual TX user[%d]: pilot power=%u", userIndex, (uint32_t)pilotPower);
            writeLen++;
        }
    ...
    /* 5. 配置发送功率 - 为广播和每个有效用户设置功率 */
    {
        ...
        if (hasBroadcast) {
            ...
            for (uint8_t i = 0; i < 16; i++) {
                if(g_brdBuffers[i].valid){
                    s_tx_pow_ctrl tx_pow_ctrl;
                    tx_pow_ctrl.data = TK8710_S_TX_POW_CTRL_ENCODE(g_brdBuffers[i].txPower, 128U + i);
                    ...
                    TK8710_LOG_IRQ_DEBUG("Manual TX broadcast[%d] power set to %d (index: %d)", 
                                        i, g_brdBuffers[i].txPower, 128 + i);
                }
            }
        }
        ...
        for (uint8_t i = 0; i < validUserCount; i++) {
            ...
            /* SetInfo and TX buffers use the packed Slot3 user position. */
            uint8_t powerUserIndex = (uint8_t)(i + slotCfg->brdUserNum);
            tx_pow_ctrl.data = TK8710_S_TX_POW_CTRL_ENCODE(power, powerUserIndex);
            ...
            if (g_manualTxPowerDiagCount < 8U) {
                s_tx_pow_ctrl readback;

                readback.data = 0U;
                if (TK8710ReadReg(TK8710_REG_TYPE_GLOBAL,
                                  MAC_BASE + offsetof(struct mac, tx_pow_ctrl),
                                  &readback.data) == TK8710_OK) {
                    TK8710_LOG_IRQ_WARN("Manual TX power map: source=%u packed=%u power=%u readbackIndex=%u readbackPower=%u",
                                        userIndex, powerUserIndex, power,
                                        (unsigned int)TK8710_S_TX_POW_CTRL_USERINDEX_GET(readback.data),
                                        (unsigned int)TK8710_S_TX_POW_CTRL_POWER_GET(readback.data));
                }
                g_manualTxPowerDiagCount++;
            }
    ...
    /* 配置广播用户valid位 (MAC init_17寄存器) */
    {
        ...
        uint32_t brdUserValid = (1U << totalBrdUsers) - 1U;
        brdUserVal.data = TK8710_S_INIT_17_ENCODE(brdUserValid);
        
        TK8710_LOG_IRQ_DEBUG("Broadcast user valid: brdUserNum=%d, slot3SharedBrdCount=%d, total=%d, val=0x%04X",
                            slotCfg->brdUserNum, slot3SharedBrdCount, totalBrdUsers, brdUserValid);
    ...
    /* 配置所有用户的MAC寄存器有效位 */
    {
        ...
        for (int i = 0; i < validUserCount; i++) {
            ...
            if (g_txBuffers[origUserIndex].valid) {
                ...
                /* 检查数据长度 */
                if (dataLen < expectedLen) {
                    if (TK8710PadTxUserData(&g_txBuffers[origUserIndex],
                                            origUserIndex,
                                            expectedLen) != TK8710_OK) {
                        errorCount++;
                        continue;
                    }
                    ...
                    TK8710_LOG_IRQ_DEBUG("Manual TX user[%u] data truncated: actual=%u expected=%u",
...
static void tk8710_s1_broadcast_tx_process(void)
{
    ...
    {
        int ret;
        uint8_t* freqBuf = g_irqInfoScratch;
    ...
    {
        int ret;
        uint8_t* ahBuf = g_irqInfoScratch;
    ...
    {
        int ret;
        uint8_t* powBuf = g_irqInfoScratch;
        uint16_t writeLen = slotCfg->brdUserNum * 5;  /* 每个用户5字节Pilot功率 */
        memset(powBuf, 0, writeLen);
        /* 为每个广播用户配置Pilot功率默认值261720U */
        for (uint8_t i = 0; i < slotCfg->brdUserNum; i++) {
    ...
    {
        int ret;
        uint8_t* anoiseBuf = g_irqInfoScratch;
    ...
    /* 配置广播发送功率 */
    {
        ...
        for (uint8_t i = 0; i < slotCfg->brdUserNum; i++) {
            ...
            tx_pow_ctrl.data = TK8710_S_TX_POW_CTRL_ENCODE(power, userIndex);
...
static void tk8710_md_data_read_signal_info(void)
{
    ...
    uint8_t maxUsers = g_irqResult.maxUsers;
    uint16_t readLen;
    uint8_t* rxBuf = g_irqSignalScratch;  /* 最大128*5*8=5120 bytes (AH数据) */
    int i;
    uint8_t validUserCount = 0;
    ...
    if (ret == 0) {
        for (i = 0; i < maxUsers; i++) {
            if (g_irqResult.crcResults[i].crcValid) {
                ...
                uint32_t snrRssi = tk8710_read_be32(&rxBuf[i * 4U]);
    ...
    if (TK8710GetTxBeamCtrlMode() == 1) {
        ...
        for (i = 0; i < maxUsers; i++) {
            if (g_irqResult.crcResults[i].crcValid && g_userInfoRxBuffers[i].valid) {
                ...
                /* 参考SPI读取时的字节顺序组合方式 */
                uint32_t freqRaw = g_userInfoRxBuffers[i].freq;
        ...
        if (ret == 0) {
            for (i = 0; i < maxUsers; i++) {
                if (g_irqResult.crcResults[i].crcValid) {
                    /* 按字节顺序直接组合频率数据 */
                    uint32_t freqRaw = tk8710_read_be32(&rxBuf[i * 4U]);
