...
static uint8_t ConvertSweepModeToTk8710Rate(int sweep_mode)
{
    switch (sweep_mode) {
        ...
        default:
            printf("[SWEEP] Unknown sweep mode: %d, use default mode 8\n", sweep_mode);
...
static int DoFrequencySweep(uint32_t start_freq, uint32_t end_freq, int sweep_mode)
{
    if (start_freq == 0 || end_freq == 0 || start_freq > end_freq) {
        printf("[SWEEP] Error: invalid frequency parameters\n");
        return -1;
    }

    if (sweep_mode < 0 || sweep_mode > 3) {
        printf("[SWEEP] Error: invalid sweep mode %d\n", sweep_mode);
        return -1;
    }

    uint8_t rate_mode = ConvertSweepModeToTk8710Rate(sweep_mode);
    printf("[SWEEP] Sweep mode: %d, rate mode: %d\n", sweep_mode, rate_mode);
    printf("[SWEEP] Start configuring sweep prerequisites\n");
    ...
    if (hal_ret != TK8710_HAL_OK) {
        printf("[SWEEP] HAL init failed: %d\n", hal_ret);
        return -1;
    }
    ...
    if (hal_ret != TK8710_HAL_OK) {
        printf("[SWEEP] HAL slot config failed: %d\n", hal_ret);
        return -1;
    }

    hal_ret = TK8710HalStart();
    if (hal_ret != TK8710_HAL_OK) {
        printf("[SWEEP] HAL start failed: %d\n", hal_ret);
        return -1;
    }

    ret = TRM_StartFrequencySweep(start_freq, end_freq, (uint8_t)sweep_mode, rate_mode);
    if (ret != TRM_OK) {
        printf("[SWEEP] TRM_StartFrequencySweep failed: %d\n", ret);
        return ret;
    }

    printf("[SWEEP] Frequency sweep started: start=%u Hz, end=%u Hz, mode=%d\n",
