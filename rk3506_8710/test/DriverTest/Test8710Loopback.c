 ...
 * @brief 8710Loopback 主测试程�?
 * @note 完整的初始化、配置、工作和中断处理流程,运行完Test8710Loopback.c后需要硬件复位一下才�?
 * 
 * RK3506 编译方法:
                ...
                for (uint8_t userIndex = 0; userIndex < TK8710_MAX_DATA_USERS; userIndex++) {
                    ...
                    if (userIndex < 128 && irqResult->crcResults[userIndex].crcValid) {
                        ...
                        if (TK8710GetRxUserSignalQuality(userIndex, &rssi, &snr, &freqSignal) == TK8710_OK) {
                            ...
                            /* 只打印前10个用户的信息 */
                            if (userIndex < 10) {
                                printf("??%u: freq=%dHz (raw=0x%08X), rssi=%d, snr=%u, ??=%u/%u\n", 
...
static int __attribute__((unused)) perform_full_initialization(void)
{
    ...
    ChipConfig chipConfig = {
        ...
        .irq_ctrl1   = 0,
        .spiConfig   = NULL,
        .rfConfig    = NULL  /* RF配置在TK8710Init中自动调�?*/
    };
...
static void run_automatic_test(void)
{
    ...
    if (g_resultFile == NULL) {
        printf("????????: 8710LoopbackTestResult.txt\n");
        return;
    }
    
    /* 写入文件�?*/
    fprintf(g_resultFile, "TK8710 Loopback ??????\n");
    fprintf(g_resultFile, "=====================================\n\n");
    fprintf(g_resultFile, "??\tSlot1\tSlot3\tClass\tCase\t??\t????\t????\t??\n");
    fprintf(g_resultFile, "----\t-----\t-----\t-----\t----\t----\t--------\t--------\t----\n");
    ...
    /* 写入总结 */
    fprintf(g_resultFile, "\n=====================================\n");
    fprintf(g_resultFile, "????: %d\n", totalTests);
    fprintf(g_resultFile, "????: %d\n", passedTests);
    fprintf(g_resultFile, "????: %d\n", totalTests - passedTests);
    fprintf(g_resultFile, "???? %.2f%%\n", (float)passedTests * 100.0f / totalTests);
