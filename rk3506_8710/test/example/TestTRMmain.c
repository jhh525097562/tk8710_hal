...
int main(int argc, char* argv[])
{
    ...
    if (use_multi_rate) {
        ...
        TRM_SlotCalcInput slotInput = {
            ...
            .brdBlockNum = slotCfg.s1Cfg[0].byteLen / 26,  // 广播包块�?
            .ulBlockNum = slotCfg.s2Cfg[0].byteLen / 26,     // 上行包块�?
            .dlBlockNum = slotCfg.s3Cfg[0].byteLen / 26,   // 下行包块�?
            .superFrameNum = 10
        };
        
    TRM_SlotCalcOutput slotOutput;
    if (trm_calc_slot_config(&slotInput, &slotOutput) == 0) {
        printf("??????%d????gap??: BRD=%u, UL=%u, DL=%u\n", 
